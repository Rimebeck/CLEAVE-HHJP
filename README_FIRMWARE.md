...
- **ファームウェア**: ZMK v0.3 (Zephyr Mechanical Keyboard)
...
# 左側ビルド
docker run --rm -v $(pwd):/workspace -w /workspace zmkfirmware/zmk-build-arm:stable \
  west build -s zmk/app -b seeeduino_xiao_ble -- -DSHIELD="cleave_hhjp_left rgbled_adapter" -DZMK_CONFIG=/workspace/config
...
# 右側ビルド
docker run --rm -v $(pwd):/workspace -w /workspace zmkfirmware/zmk-build-arm:stable \
  west build -s zmk/app -b seeeduino_xiao_ble -- -DSHIELD="cleave_hhjp_right rgbled_adapter" -DZMK_CONFIG=/workspace/config
