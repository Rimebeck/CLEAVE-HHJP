...
- **ファームウェア**: ZMK main (Zephyr Mechanical Keyboard)
...
# 左側ビルド
docker run --rm -v $(pwd):/workspace -w /workspace zmkfirmware/zmk-build-arm:stable \
  west build -s zmk/app -b "xiao_ble//zmk" -- -DSHIELD="cleave_hhjp_left rgbled_adapter" -DZMK_CONFIG=/workspace/config
...
# 右側ビルド
docker run --rm -v $(pwd):/workspace -w /workspace zmkfirmware/zmk-build-arm:stable \
  west build -s zmk/app -b "xiao_ble//zmk" -- -DSHIELD="cleave_hhjp_right rgbled_adapter" -DZMK_CONFIG=/workspace/config
...
6. `cleave_hhjp_left-xiao_ble-zmk.uf2` をドライブにコピー
...
2. USB Type-CケーブルでPCに接続
3. XIAOのリセットボタンを**ダブルクリック**
4. `cleave_hhjp_right-xiao_ble-zmk.uf2` をコピー
5. 自動的に再起動
...
| 1 | Fn | Fnキー押下時（F1-F12、メディア、ナビゲーション、BT切替） |

### Bluetooth切替

FnレイヤーでCtrlキーを押しながらF1-F5を押すと、Bluetoothプロファイル1-5に切替（mod-morph）。Ctrl+DELでペアリングクリア。
...
- **Bluetooth 5デバイス切替**: 最大5台のデバイスをペアリング（Fnレイヤーで Ctrl+F1〜F5）
- **バッテリー残量表示**: 内蔵RGB LED
...
│   └── west.yml                        # 依存関係定義（ZMK main）
├── build.yaml                          # ビルドターゲット定義
└── README_FIRMWARE.md                  # このファイル
```
