...
この章ではCLEAVE HHJPのキーマップやファームウェア設定を変更する方法と、カスタムケースを作りたい場合に参照できるPCBの3Dデータの場所をまとめます。

> [!NOTE]
> 初期キーマップと配布ケースのまま使う場合、このガイドの作業は必須ではありません。
> キー配置、ファームウェア設定、PCB外形データを確認したくなったときに参照してください。
...
- [PCB外形データを使ってオリジナルケースをデザインする](#pcb外形データを使ってオリジナルケースをデザインする)
...
| 目的 | ファイル |
|---|---|
| キー割り当てを変更する | [`cleave_hhjp.keymap`](../config/cleave_hhjp.keymap) |
...
|---|---|---|---|
| キーを入れ替えたい | [`cleave_hhjp.keymap`](../config/cleave_hhjp.keymap) | はじめて向け | まずはこのファイルだけを変更します。 |
| Fnレイヤーの割り当てを変えたい | [`cleave_hhjp.keymap`](../config/cleave_hhjp.keymap) | はじめて向け | `fn_layer`内の割り当てを変更します。 |
...
## PCB外形データを使ってオリジナルケースをデザインする

カスタムケースを作りたい場合は、左右PCBの外形データとして[left.step](../models/pcb/left.step)と[right.step](../models/pcb/right.step)を参照できます。これらはKiCadから出力したmm単位のSTEPファイルです。

> [!IMPORTANT]
> PCB外形データは参照用です。カスタムケースの設計・製作は自己責任で行ってください。
