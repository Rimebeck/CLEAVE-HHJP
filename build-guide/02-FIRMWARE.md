...
3. 右側のXIAO nRF52840 Plusの内蔵LEDを確認します。右側のLEDは左右のキーボード間の接続状態を示し、左側と接続済みになると青色、未接続のときは赤色に点灯します。これは充電LEDとは別の表示です。
4. PCやスマートフォンのBluetooth設定を開き、`CLEAVE HHJP`を選択して接続します。
5. 左側のXIAO nRF52840 Plusの内蔵LEDを確認します。左側のLEDはPCやスマートフォンとの接続状態を示し、ファーム書き込み直後など空のBluetoothプロファイルでペアリング待ちになっている間は黄色、接続済みになると青色に点灯します。
