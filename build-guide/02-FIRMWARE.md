...
XIAO nRF52840 Plusをブートローダーモードにすると、`XIAO-SENSE`（別の名前で認識される場合があります）という名前のUSBドライブとして認識されます。

1. PCと書き込み対象のXIAO nRF52840 PlusをUSB Type-Cケーブルで接続します。
2. RESETボタンを爪楊枝の柄側など尖りすぎていない道具を使って素早く2回押します。（ダブルクリック）

    <img src="./images/firmware/reset-button/1.jpg" alt="XIAO nRF52840 PlusのRESETボタン位置" width="600px">

3. `XIAO-SENSE`（別の名前で認識される場合があります）ドライブが表示されます。
...
7. 両方の書き込みが完了したら、次の接続確認・動作確認に進みます。
> [!NOTE]
> macOSではUF2ファイルのコピー後にエラーが表示される場合がありますが、コピー後に`XIAO-SENSE`ドライブが消えて自動的に再起動していれば、書き込みは完了しています。

## 接続確認・動作確認

ケースへ組み込む前に、左右の基板がBluetoothで接続でき、仮取り付けしたキースイッチが入力として認識されることを確認します。

1. 近くに他のCLEAVE HHJPがある場合は、ほかのペアの電源を切ります。
2. 左右のUSBケーブルを抜き、左右の電源スイッチをONにします。左右間のペアリングは電源を入れると自動的に行われます。
3. 右側のXIAO nRF52840 Plusの内蔵LEDを確認します。右側のLEDは左右のキーボード間の接続状態を示し、左側と接続済みになると青色、ペアリング待ちや接続先の左側を探している間は黄色、未接続のときは赤色に一回点灯します。これは充電LEDとは別の表示です。
4. PCやスマートフォンのBluetooth設定を開き、`CLEAVE HHJP`を選択して接続します。
5. 左側のXIAO nRF52840 Plusの内蔵LEDを確認します。左側のLEDはPCやスマートフォンとの接続状態を示し、ファーム書き込み直後など空のBluetoothプロファイルでペアリング待ちになっている間は黄色、接続済みになると青色に一回点灯します。
6. テキストエディタやキーボードテストツールを開き、基板組み立てガイドで仮取り付けしたキースイッチを1つずつ押します。
7. 左右どちらの基板に取り付けたキースイッチも入力できることを確認します。すべての位置にスイッチを仮取り付けしている場合は、左右すべてのキーを確認します。

`CLEAVE HHJP`が表示されない、接続できない、または一部のキーが反応しない場合は、[トラブルシューティング](#トラブルシューティング)を確認してください。

> [!NOTE]
> 左右のペアリングは、ZMKがシールド名から生成するBluetooth UUIDを使って自動的に行われます。複数のCLEAVE HHJPを同時にセットアップすると意図しない左右の組み合わせでペアリングされる可能性があるため、初回セットアップ時やペアリングリセット後は1ペアずつ電源を入れてください。一度正しくペアリングが確立すれば、以降は複数ペアを同時に使用しても問題ありません。
...
- PCやスマートフォン側に古い`CLEAVE HHJP`の登録が残っている場合は、OS側の登録を削除してから再検索します。キーボード側のペアリング情報も消す場合は`Ctrl` + `Fn` + `Backspace`で現在のプロファイルをクリアします。
...
続いて、準備ガイドで決めたケース方式のガイドへ進み、常用できる状態に仕上げます。
- [前: 基板組み立てガイド](01-ASSEMBLY.md)
- [次（標準ケース）: 標準ケースビルドガイド](03-0-STANDARD_CASE.md)
- [次（簡易ケース）: 簡易ケースビルドガイド](03-1-SIMPLE_CASE.md)
- [次（オリジナルケース）: オリジナルケース作成ガイド](03-2-CUSTOM_CASE.md)
- [目次に戻る](README.md)
