...
| ケース部品の用意方法 | 自家プリント / 3Dプリントサービスに発注 | 3Dプリント部品、ケース部材、組み立て手順 |
...
| バッテリー | LiPoバッテリー JSTオスコネクタ付き | 2 |  | このケースは最大102050サイズ（厚み10mm × 幅20mm × 長さ50mm）のバッテリーまで対応 |
| マイコンボード | XIAO nRF52840 Plus | 2 |  | 左右両側に1個ずつ。"Plus"が必要です。購入先例: [スイッチサイエンス](https://www.switch-science.com/products/10468) |
| 位置合わせ用 | ピンヘッダ（2.54mmピッチ） | 2 |  | XIAO nRF52840 Plus実装時の位置合わせに一時的に使用します。本実装には使用しません。ロットによりXIAO nRF52840 Plusに付属していることがあります。 |
...
Cleave HHJPは、MX互換スイッチとChocV1/V2互換スイッチの両方に対応しており、一度組み立てた後も気分によって切り替えることも可能です。
...
| フォームテープ | フォームテープ2mm厚/3mm幅 | 2m程度 |  | 購入例: [フォームテープ](https://www.amazon.co.jp/dp/B0CG9N7B5M)<br>購入例は幅5mmのため、半分程度に割いて使用します。 |
...
- [ ] 基板、ダイオード、電源スイッチ、バッテリーコネクタ、XIAO nRF52840 Plus、位置合わせ用ピンヘッダ、LiPoバッテリーを用意した
- [ ] 使用するスイッチに対応したホットスワップソケット、キースイッチ、キーキャップを用意した
- [ ] MX構成で必要な場合はPCBマウント2Uスタビライザーを用意した
- [ ] ケース用の3Dプリント部品を用意した
- [ ] インサートナット、M2ネジ、フォームテープ、バッテリー固定用両面テープ、必要に応じて透明プラ棒と滑り止めシートを用意した
