...
| ケース作成方法 | 標準ケース（3Dプリント） / 簡易ケースキット / オリジナルケースを設計 | ケース部品、ケース部材、参照するケースガイド |
| 標準ケース選択時の部品用意方法と素材 | 自家プリント / 3Dプリントサービス（JLC3DP等）に発注 | 標準ケース用3Dプリント部品、ケース部材 |
| 標準ケース選択時のトップカバータイプ | インジケーター穴あり / 穴なし | トップカバーのモデル、透明プラ棒の有無 |
...
| 電源スイッチ | MSK-12C02 2.5H | 2 | ◯ | ノブが高い2.5Hを推奨 |
| スイッチングダイオード | 1N4148W (SOD-123) | 70 | ◯ | チップ表面に「T4」と表記があるもの |
| ショットキーバリアダイオード | B5819W (SOD-123) | 2 | ◯ | チップ表面に「SL」と表記があるもの |
| バッテリーコネクタ | PHコネクター ベース付ポスト サイド型 2P | 2 | ◯ |  |
| 位置合わせ用 | 1x2ピンヘッダ（2.54mmピッチ） | 2 | ◯ | XIAO nRF52840 Plus実装時の位置合わせに一時的に使用します。本実装には使用しません。ロットによりXIAO nRF52840 Plusに付属していることがあります。 |
| バッテリー | LiPoバッテリー JST PHコネクタ付き | 2 |  | 標準ケース・簡易ケースキットは最大102050サイズ（厚み10mm × 幅20mm × 長さ50mm）のバッテリーまで対応。購入先例: [Amazon](https://www.amazon.co.jp/dp/B0GFD6DQPP) |
| マイコンボード | XIAO nRF52840 Plus | 2 |  | 左右両側に1個ずつ。"Plus"が必要です。購入先例: [スイッチサイエンス](https://www.switch-science.com/products/10468) / [遊舎工房](https://shop.yushakobo.jp/products/10946) / [seeedstudio公式ストア](https://jp.seeedstudio.com/Seeed-Studio-XIAO-nRF52840-Plus-p-6359.html) |
...
CLEAVE HHJP のケース作成方法は、標準ケース、簡易ケースキット、オリジナルケースの3つです。
どの方法を選ぶかによって、準備するケース部品や参照するガイドが変わります。

| 方法 | 説明 | 用意するもの | 参照先 |
|---|---|---|---|
| 標準ケース（3Dプリント） | 配布されているSTLを3Dプリントしてケースを作成する方式です。 | 配布STL、3Dプリント部品、インサートナット、M2ネジ、フォームテープなど | [標準ケースビルドガイド](03-0-STANDARD_CASE.md) |
| 簡易ケースキット | ボトムプレートとトッププレートでPCBを挟み込む、サンドイッチ構造のケースキットを使う方式です。 | 簡易ケースキット、M2ネジ、スペーサー固定用の両面テープなど | [簡易ケースビルドガイド](03-1-SIMPLE_CASE.md) |
| オリジナルケース（STEP参照） | PCB外形STEPデータを元に、自分でケース構造を設計・製作する方式です。 | PCB外形STEPデータ、CAD環境、独自に設計・製作したケース部品 | [オリジナルケース作成ガイド](03-2-CUSTOM_CASE.md) |

ここで決めたケース方式に応じて、ファームウェア書き込みと基板単体確認のあとに該当するケースガイドへ進みます。

### 標準ケース用3Dプリント部品

標準ケースを作る場合は、以下の3Dプリント部品を用意します。使用するキースイッチとトップカバーのタイプに合わせて、表の選択条件から必要なモデルを選びます。

後からMX/Chocを切り替えたい場合は、両方のトッププレートを用意しておきます。
...
簡易ケースキットを使う場合は、ここに記載している標準ケース用3Dプリント部品は使用しません。キット内容物は[簡易ケースビルドガイド](03-1-SIMPLE_CASE.md#1-簡易ケース部品の確認)で確認してください。
オリジナルケースを作る場合は、[PCB外形STEPデータ](../models/pcb)を参照して独自に設計します。

### 標準ケースのベースケース素材の選び方

標準ケースの左右のベースケースは、ケース組み立て時にインサートナットをはんだごてで熱圧入するため、ベースケースには熱可塑性樹脂を選びます。
...
### 標準ケースで3Dプリント部品以外に用意する部材

標準ケースでは、3Dプリントしたケース部品に加えて、以下の部材も必要になります。
...
- [ ] ケース作成方法（標準ケース / 簡易ケースキット / オリジナルケース）を決めた
- [ ] 標準ケースを選ぶ場合は、ケース部品の用意方法（自家プリント / 3Dプリントサービスに発注）を決めた
- [ ] 標準ケースを選ぶ場合は、トップカバーのタイプ（インジケーター穴あり / 穴なし）を決めた
- [ ] 必要な工具とUSB Type-Cケーブルを用意した
- [ ] 必要な部品をすべて揃えた

準備ができたら、次のガイドに進んでください。

- [次: 基板組み立てガイド](01-ASSEMBLY.md)
- [目次に戻る](README.md)
