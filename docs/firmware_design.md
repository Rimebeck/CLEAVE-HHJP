...
| 予備 | D13 | P0.13 | P0.13 | **端面TH** / 充電電流制御 |
...
| 状態 | 色 | パターン | 表示タイミング |
|------|-----|----------|----------------|
| 高バッテリー残量 (>80%) | 緑 | 約1秒点灯 | 起動時 |
| 中バッテリー残量 (20-80%) | 黄 | 約1秒点灯 | 起動時 |
| BLE接続済み | 青 | 約1秒点灯 | 接続確立時 |
| 低バッテリー残量 (<20%) | 赤 | 約1秒点灯 | 起動時または警告表示時 |
| 未接続 | 赤 | 約1秒点灯 | 切断時 |
...
|------|------|
| 充電IC | BQ25101 (XIAO内蔵) |
| 充電電流 | 100mA（P0.13 output-low による ISET 並列抵抗制御） |
| 充電方式 | USB Type-C経由 |
| 充電制御 | 自動（CC/CV方式） |

**充電電流の仕組み**:

BQ25101 の ISET ピンには固定 2.7kΩ 抵抗が接続されている（XIAO 基板上）。
P0.13 も 2.7kΩ を介して ISET に接続されており、GPIO 状態で充電電流を切替可能。

| P0.13 の状態 | 合成 RISET | 充電電流 |
|---|---|---|
| Input / Hi-Z（デフォルト） | 2.7kΩ | 50mA |
| Output LOW（ファームウェア設定） | 2.7k ∥ 2.7k = 1.35kΩ | 100mA |
| Output HIGH（**使用禁止**） | N/A（ISET 異常） | 0mA（充電停止） |
...
- スイッチOFF: バッテリーからの給電は停止。USB Type-C接続時はUSB給電により充電・動作可能
...
GitHub Actionsではなくローカルで確認したい場合は、ZMK公式Dockerイメージを使用できます。以下はリポジトリの最上位ディレクトリで実行する例です。
...
4. **書き込み完了**
   ...
   - 左側キーボードが自動的に再起動
   - User LEDが接続状態に応じた色に約1秒点灯（BLEアドバタイジング開始）
...
- [ ] 左側キーボード電源ON後、User LEDが接続状態に応じた色に約1秒点灯する
- [ ] 右側キーボード電源ON後、User LEDが接続状態に応じた色に約1秒点灯する
- [ ] 左右が自動接続し、User LEDが青色に約1秒点灯する（接続済み）
...
- [ ] USB充電時、充電LEDが緑色点灯（充電中。電源スイッチOFFでもUSB給電中は動作可能）
- [ ] USB充電完了後、充電LEDが消灯（充電完了）
...
| 条件 | 期待されるLED表示 |
|------|-------------------|
| 電源ON直後（バッテリー>80%） | 緑色に約1秒点灯 |
| BLEアドバタイジング開始時 | 黄色に約1秒点灯 |
| BLE接続済み | 青色に約1秒点灯 |
| バッテリー残量<20% | 赤色に約1秒点灯 |
| USB充電中 | 充電LED緑色点灯 |
| USB充電完了 | 充電LED消灯 |
